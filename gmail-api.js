...
class GmailSender {
    ...
    // ============================================
    // 🔧 ENVÍO DE EMAIL - VERSIÓN CORREGIDA
    // ============================================
    async sendEmail(to, subject, htmlContent, attachments = []) {
        ...
        // 🔧 CORRECCIÓN: Usar encodeURIComponent para caracteres especiales
        let encodedEmail;
        try {
            encodedEmail = btoa(unescape(encodeURIComponent(email)))
                .replace(/\+/g, '-')
                .replace(/\//g, '_')
                .replace(/=+$/, '');
        } catch (e) {
            // Si falla, usar método alternativo
            console.warn('⚠️ Falló btoa con encodeURIComponent, usando método alternativo...');
    ...
    buildEmailMime(to, subject, htmlContent, attachments = []) {
        ...
        // 🔧 CORRECCIÓN: Codificar asunto correctamente
        let encodedSubject;
        try {
            encodedSubject = btoa(unescape(encodeURIComponent(subject)));
        } catch (e) {
            encodedSubject = btoa(subject);
        }
