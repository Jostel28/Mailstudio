...
function updateAttachmentsPreview() {
    ...
    let html = '<div style="background:#f1f5f9; padding:8px; border-radius:8px;"><strong>📎 Adjuntos:</strong><br>';
    attachments.forEach(file => {
        let icon = file.type.startsWith('image/') ? '🖼️' : (file.type.startsWith('video/') ? '🎥' : '📄');
        html += `<span style="display:inline-block; margin:4px;">${icon} ${file.name}</span><br>`;
    });
    html += '</div>';
    previewAttachmentList.innerHTML = html;
}
...
fileAttachment.addEventListener('change', (e) => { 
    attachments = [...attachments, ...Array.from(e.target.files)]; 
...
async function sendWithGmail() {
    ...
    if (!confirm(`📊 Vas a enviar ${total} correos.\n\n` +
        ...
        `⏱️ Tiempo estimado: ~${timeEstimate} segundos\n` +
        ...
        const finalHtml = `
            <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
                ...
                <div style="padding: 20px;">
                    ...
                    <div>${previewBody.innerHTML}</div>
                </div>
                <div style="text-align: center; padding: 20px; background: #f8fafc; font-size: 12px;">
                    ${previewBottomLogo.src && !previewBottomLogo.src.includes('placehold') ? 
                        `<img src="${previewBottomLogo.src}" style="max-height: 50px;"><br>` : ''}
